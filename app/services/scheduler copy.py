...
class TaskScheduler:
    def __init__(
        ...
        self.adb_transfer = ADBTransferService()
    ...
    async def process_task(self, task: Task, db: Session) -> bool:
        ...
        while retry_count < self.max_retries:
            try:
                success = await self.adb_transfer.execute_transfer(task, db)
                if success:
                    return True
                    
                retry_count += 1
                logger.warning(f"任务 {task.id} 解锁失败，第 {retry_count} 次重试")
